import sys  # Path: Pdf_Manager.py
import os  # imorting os.path
import io
...
class AppDemo(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('PDF Desktop Manager')
    ...
    def initUI(self):
        ...
        Buttons
        """
        self.buttonAdd = button('&Add')
        self.buttonAdd.setFixedHeight(self.outputFile.height)
        self.buttonAdd.clicked.connect(self.populateFileName)
        ...
        self.buttonSpilt = button('&Split')
        # buttonSpilt.setIcon(QIcon('./Icons/play.ico'))
        self.buttonSpilt.clicked.connect(self.splitFile)
        buttonLayout.addWidget(self.buttonSpilt)
    ...
    def _getSaveFilePath(self):
        file_save_path, _ = QFileDialog.getSaveFileName(
            self, 'Save PDF file', os.getcwd(), 'PDF file (*.pdf)')
    ...
    def mergeFile(self):
        ...
        if self.pdfListWidget.count() > 0:
            ...
            try:
                ...
                self.pdfListWidget.clear()
                self.dialogMessage('PDF merge complete. Kindly Check PDF')

            except Exception as e:
                self.dialogMessage(e)
        else:
            self.dialogMessage('Queue is empty. Kindly Add Pdf to Merge!')
